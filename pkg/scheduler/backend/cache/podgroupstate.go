...
func (pgk podGroupKey) String() string {
	return string(pgk.keyType) + "/" + pgk.namespace + "/" + pgk.name
}
...
func (d *compositePodGroupStateData) addChild(child *podGroupKey) {
	d.generation = nextPodGroupGeneration()
	d.children.Insert(*child)
}

// removeChild removes a child group from this group.
func (d *compositePodGroupStateData) removeChild(child *podGroupKey) {
	d.generation = nextPodGroupGeneration()
	d.children.Delete(*child)
}

// getChildren returns all child groups for this group.
func (d *compositePodGroupStateData) getChildren() []string {
	var children []string
...
func (cpgs *compositePodGroupState) GetChildren() []string {
	cpgs.lock.RLock()
	defer cpgs.lock.RUnlock()
...
func (s *compositePodGroupStateSnapshot) GetChildren() []string {
