	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, pod := range tt.pods {
				...
				key := fmt.Sprintf("%s/%s/%s", fwk.PodGroupKeyType, tt.podGroup.Namespace, tt.podGroup.Name)
...
func TestSkipPodGroupPodSchedule(t *testing.T) {
	...
	podGroupInfo := &framework.QueuedPodGroupInfo{
		QueuedPodInfos: map[string][]*framework.QueuedPodInfo{"podgroup/default/pg": {qInfo1, qInfo2, qInfo3}},
		PodGroupInfo: &framework.PodGroupInfo{
			Name:            "pg",
			Namespace:       "default",
			Type:            fwk.PodGroupKeyType,
	...
	if podGroupInfo.QueuedPodInfos["podgroup/default/pg"][0].Pod.Name != "p1" {
		t.Errorf("Expected p1 to be left in queued pods, got %s", podGroupInfo.QueuedPodInfos["podgroup/default/pg"][0].Pod.Name)
	}
...
func TestPodGroupCycle_UpdateSnapshotError(t *testing.T) {
	...
	podGroupInfo := &framework.QueuedPodGroupInfo{
		QueuedPodInfos: map[string][]*framework.QueuedPodInfo{"podgroup/default/pg": {qInfo1, qInfo2}},
		PodGroupInfo: &framework.PodGroupInfo{
			Name:            "pg",
			Namespace:       "default",
			Type:            fwk.PodGroupKeyType,
...
func TestPodGroupCycle_FillsPodResultsOnFewerResults(t *testing.T) {
	...
	podGroupInfo := &framework.QueuedPodGroupInfo{
		QueuedPodInfos: map[string][]*framework.QueuedPodInfo{"podgroup/default/pg": queuedPodInfos},
		PodGroupInfo: &framework.PodGroupInfo{
			Name:            "pg",
			Namespace:       "default",
			Type:            fwk.PodGroupKeyType,
...
func TestSubmitPodGroupAlgorithmResult(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sched.submitPodGroupAlgorithmResult(ctx, schedFwk, podGroupCycleState, podGroupInfo, []podGroupAlgorithmResult{tt.algorithmResult}, time.Now())
...
func TestCPGHierarchicalScheduling_ScheduleOnePodGroup(t *testing.T) {
	...
	qInfo2 := &framework.QueuedPodInfo{PodInfo: &framework.PodInfo{Pod: p2}}
	qInfo3 := &framework.QueuedPodInfo{PodInfo: &framework.PodInfo{Pod: p3}}
	queuedPodInfosMap := map[string][]*framework.QueuedPodInfo{
		"podgroup/default/pg1": {qInfo1},
		"podgroup/default/pg2": {qInfo2},
		"podgroup/default/pg3": {qInfo3},
	}
	...
	res, _, status := sched.podGroupSchedulingRecursiveAlgorithm(ctx, schedFwk, framework.NewCycleState(), cpgRootInfo, cpgRootInfo.PodGroupInfo, runAllPostFilters)
...
func TestCPGHierarchicalScheduling_Internal(t *testing.T) {
	...
	createPods := func(pgName string, count int) {
		for i := 0; i < count; i++ {
			...
			key := fmt.Sprintf("%s/%s/%s", fwk.PodGroupKeyType, ns, pgName)
