...
func testDeclarativeValidate(t *testing.T, apiVersion string) {
	...
	testCases := map[string]struct {
		...
		"too high priority, workload aware preemption enabled": {
			...
			enableWorkloadAwarePreemption: true,
			expectedErrs: field.ErrorList{
				field.Invalid(field.NewPath("spec", "podGroupTemplates").Index(0).Child("priority"), scheduling.HighestUserDefinablePriority+1, "must be less than or equal to 1000000000").WithOrigin("maximum"),
			},
		},
...
func testDeclarativeValidateUpdate(t *testing.T, apiVersion string) {
	testCases := map[string]struct {
		...
		"set controllerRef": {
			...
			expectedErrs: field.ErrorList{field.Invalid(field.NewPath("spec", "controllerRef"), nil, "field is immutable").WithOrigin("immutable")},
		},
		...
		"invalid update empty podGroupTemplates": {
			...
			expectedErrs: field.ErrorList{
				...
				field.Invalid(field.NewPath("spec", "podGroupTemplates"), []scheduling.PodGroupTemplate{}, "field is immutable").WithOrigin("immutable"),
			},
		},
		"invalid update too many podGroupTemplates": {
			...
			expectedErrs: field.ErrorList{
				...
				field.Invalid(field.NewPath("spec", "podGroupTemplates"), nil, "field is immutable").WithOrigin("immutable"),
			},
		},
